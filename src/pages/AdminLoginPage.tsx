...
import { LogIn, ShieldCheck, AlertCircle, Eye, EyeOff, UserCircle } from 'lucide-react';
import { adminLogin, getDemoAdminAccounts } from '@/lib/adminAuth';

interface Props {
  onLogin: (session: { id: string; email: string; name: string }) => void;
  onGoHome: () => void;
}

export function AdminLoginPage({ onLogin, onGoHome }: Props) {
  const [id, setId] = useState('');
  const [password, setPassword] = useState('');
  const [showPw, setShowPw] = useState(false);
  const [error, setError] = useState('');
  const [loading, setLoading] = useState(false);
  const demoAccounts = getDemoAdminAccounts();
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    setTimeout(() => {
      const account = adminLogin(id.trim(), password);
      if (!account) {
        setError('Invalid admin credentials. Use a demo account below.');
        setLoading(false);
        return;
      }
      onLogin({ id: account.id, email: account.email, name: account.name });
      setLoading(false);
    }, 350);
  }
  ...
  return (
    <div className="mx-auto max-w-md">
      <div className="mb-6 text-center">
        <div className="mx-auto mb-3 flex h-14 w-14 items-center justify-center rounded-2xl bg-accent-600 shadow-lg shadow-accent-600/30">
          <ShieldCheck className="h-7 w-7 text-white" />
        </div>
        <h1 className="text-2xl font-bold text-white">Admin Login</h1>
        <p className="mt-1 text-sm text-secondary-400">Emergency Control Center access — authorized personnel only.</p>
      </div>

      <div className="card p-5 sm:p-6">
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="label" htmlFor="aid">Admin ID / Email</label>
            <input
              id="aid"
              className="input"
              value={id}
              onChange={(e) => setId(e.target.value)}
              placeholder="e.g. A-001 or admin@alertx.demo"
              autoComplete="username"
            />
          </div>
          <div>
            <label className="label" htmlFor="apw">Password</label>
      ...
      <div className="card mt-4 p-4">
        <div className="mb-2 flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-secondary-400">
          <UserCircle className="h-4 w-4" /> Demo Admin Accounts
        </div>
